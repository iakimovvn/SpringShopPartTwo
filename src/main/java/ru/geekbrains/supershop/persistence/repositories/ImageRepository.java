...
public interface ImageRepository extends JpaRepository<Image, UUID> {
    ...
    @Query(value = "SELECT image.name FROM image INNER JOIN purchase p ON image.id = p.image WHERE p.id = :id", nativeQuery = true)
    String obtainImageNameByPurchaseId(@Param("id") UUID id);
}